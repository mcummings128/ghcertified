...
> If setting an output in a reusable workflow and passing that output to a caller workflow is desired, both a workflow-level and a job-level `outputs` block must be used. See the [official documentation](https://docs.github.com/en/actions/how-tos/reuse-automations/reuse-workflows#using-outputs-from-a-reusable-workflow) for more details.
