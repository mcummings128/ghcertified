...
my-job:
  if: ${{ vars.MY_VAR }} == 'MY_VALUE'
```
> Incorrecto, solo `vars.MY_VAR` se evalúa dentro de `${{ }}`; esto produce texto como `some_value == 'MY_VALUE'`, y GitHub trata esa cadena no vacía como verdadera en lugar de realizar la comparación  
> Consulta https://docs.github.com/en/actions/reference/workflows-and-actions/expressions#literals
- [ ] No es posible porque las variables de configuración no se pueden usar en condiciones `if`
> Esto es cierto para `secrets`, pero no para las variables de configuración
- [ ] No es posible porque las variables de configuración no se pueden usar en condiciones `if` a nivel de trabajo
> Esto es cierto para `secrets`, pero no para las variables de configuración
