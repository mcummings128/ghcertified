...
> `hashFiles` es una función integrada de GitHub que crea un hash de la ruta especificada. Usarla para componer una clave de caché provoca que el hash se regenere, lo que a su vez actualiza la clave de la caché. La documentación oficial [Dependency Caching Reference](https://docs.github.com/en/actions/reference/workflows-and-actions/dependency-caching#example-using-the-cache-action) muestra cómo usar `hashFiles` como parte de una clave de caché.
- [ ] `hashFiles` es necesario para la compatibilidad con runners de Windows.
- [ ] Al usar `hashFiles` como parte de una clave de caché, si no se encuentra la caché, `hashFiles` proporciona información adicional de depuración.  
- [ ] Al usar `hashFiles` como parte de una clave de caché, se generará un paso adicional en el flujo de trabajo del llamador. Este paso del flujo de trabajo imprime el valor del hash SHA-256 de la clave de caché como referencia.  
