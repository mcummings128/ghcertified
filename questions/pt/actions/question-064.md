---
question: "Como as organizações que estão usando o GitHub Enterprise Server podem habilitar a sincronização automática de GitHub Actions de terceiros hospedados no GitHub.com para sua instância do GitHub Enterprise Server?"
...
> Quando ativado, o GitHub Connect será usado para estabelecer automaticamente uma conexão segura entre uma instância do GitHub Enterprise Server (GHES) e uma conta do GitHub Enterprise Cloud (GHEC) (o GHEC é hospedado no GitHub.com). A conta GHEC serve como uma identidade confiável (autenticada) que o GitHub.com utiliza para autorizar a instância GHES a acessar as actions hospedadas no GitHub.com.  
...
- [ ] O GitHub Enterprise Server (GHES) não pode usar GitHub.com Actions devido à sua natureza on-premise e à falta de acesso à internet.  
> Embora os workflows localizados em uma instância GHES não possam usar GitHub.com Actions nem actions do GitHub Marketplace por padrão, isso pode ser corrigido com o uso do GitHub Connect ou actions-sync.  
